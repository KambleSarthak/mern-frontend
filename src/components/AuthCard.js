  ...
  return (
    <>
      <Form
        className=""
        onSubmit={
          isUserLogin ? handleLogin : isUserReg ? handleRegistration : null
        }
      >
        <Card
      ...
      <Modal show={showMap} onHide={() => setShowMap(false)} size="lg" centered>
        ...
        <Modal.Body style={{ height: "60vh", width: "100%" }}>
          <MapScreen
            setLat={(lat) => setLocationData((prev) => ({ ...prev, lat }))}
            setLong={(lng) => setLocationData((prev) => ({ ...prev, lng }))}
            setAddr={(addr) =>
              setLocationData((prev) => ({ ...prev, address: addr }))
            }
          />
        </Modal.Body>
        <Modal.Footer>
